        ...
        <div class="space-y-9">
          <div class="py-0.5 border-t border-b border-dashed border-gray-700 flex items-center justify-between">
            <div class="text-primary font-bold">
              <nuxt-link class="hover:text-primary" :to="localePath('/blog')">{{ $t('blog.header') }}</nuxt-link> <span class="text-gray-700">/</span> {{ post.category }}
            </div>
            ...
            <header class="py-4">
              <div class="space-y-1 text-center">
                ...
                <dl class="">
                  <div><dt class="sr-only">Published on</dt>
                    <dd class="text-xs font-medium text-gray-500">
                      <a target="_blank" rel="noreferrer" :href="`https://twitter.com/${post.author.twitter}`"><span class="text-primary hover:text-primary">{{ post.author.name }}</span></a>
                ...
                <div class="overflow-hidden flex flex-wrap items-center flex-row justify-center">
                    <div class="px-3 text-xs flex flex-shrink-0 py-1 mt-1 mr-2 bg-primary rounded-md hover:bg-indigo-700" v-for="(tag, index) in post.tags" :key="`tag-${index}`">{{ tag }}</div>
                </div>
              </div>
            </header>
